...
#ifdef USE_COFF  /* Start of "coff.h" */
/* Copyright (C) 2015 DJ Delorie, see COPYING.DJ for details */
...
line
	...
	| ID ':'			{ Symbol *s = set_symbol($1, pc); s->type |= (pc?SYM_data:SYM_code); }
	| ID '=' const			{ Symbol *s = set_symbol($1, $3); s->type = SYM_abs; }
	| STRUCT ID '\n'		{ struct_pc=0;
					  struct_tp=$1;
					  struct_sym=$2->name;
					  lineno++;
	  ...
	  struct_lines
	  ENDS				{ Symbol *s = set_symbol($2, struct_pc); s->type = SYM_abs;
					  $<sym>4->next=symtab;
					  symtab=$<sym>4;
	  				}
	| ENUM ID '\n'			{ struct_pc=0;
...
/*unsigned char *cp;*/
  ...
  if (read(f, data, (unsigned)len) != len) {
    ...
    perror("");  /* TODO(pts): Isn't this too late to print the error? */
    close(f);
    return;
  }
